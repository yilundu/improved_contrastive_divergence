...
import torch
from data import CelebAHQOverfit, CelebAHQ
...
flags.DEFINE_integer('num_steps', 120, 'number of steps to optimize the label')
...
flags.DEFINE_bool('sigmoid', False, 'sigmoid is not true')
...
def conceptcombineeval(model_list, select_idx):
    ...
    im_size = 128
    transform = transforms.Compose([transforms.RandomResizedCrop(im_size, scale=(0.3, 1.0)), transforms.RandomHorizontalFlip(), color_transform, GaussianBlur(kernel_size=5), transforms.ToTensor()])
    ...
    for _, data, label in tqdm(dataloader):
        ...
        fake_ims.extend(list((im_cpu.numpy().transpose((0, 2, 3, 1)) * 255).astype(np.uint8)))
...
def conceptcombine(model_list, select_idx):
    ...
    ims_gen = []
    w = imageio.get_writer("uncond_gen.mp4")
    # First get good initializations for sampling
    for i in range(10):
        for i in range(50):
            if i % 3 == 0:
                w.append_data(im[:9].detach().view(3, 3, 3, 128, 128).permute(0, 3, 1, 4, 2).cpu().numpy().reshape(3*128, 3*128, 3) * 255)
    ...
    # im = torch.stack(ims_gen, dim=0).permute(1, 3, 0, 4, 2)
    # output = im.detach().cpu().numpy()
    # output = im.detach().cpu().numpy()
    # output = output.reshape((8 * 128, -1, 3))
...
def combine_main(models, resume_iters, select_idx):
    ...
    for model, resume_iter in zip(models, resume_iters):
        ...
        model_base = CelebAModel(FLAGS_model, debug=True)
...
if __name__ == "__main__":
    models_orig = ['ffhq_dataset_uncond']
    resume_iters_orig = ["12000"]
