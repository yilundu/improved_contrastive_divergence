...
import torch
from data import CelebAHQOverfit, LSUNBed
...
flags.DEFINE_bool('sigmoid', False, 'simoid is not true')
...
def conceptcombineeval(model_list, select_idx):
    ...
    im_size = 128
    transform = transforms.Compose([transforms.RandomResizedCrop(im_size, scale=(0.9, 1.0)), transforms.RandomHorizontalFlip(), color_transform, transforms.ToTensor()])
    ...
    select_fake_ims = fake_ims[-100:]
    select_fake_ims = np.array(select_fake_ims)
    select_fake_ims = select_fake_ims.reshape((10, 10, 128, 128, 3)).transpose((0, 2, 1, 3, 4)).reshape((1280, 1280, 3))
    imsave("select_sample.png", select_fake_ims)
...
if __name__ == "__main__":
    models_orig = ['lsun_dataset_uncond']
    resume_iters_orig = ["30000"]
